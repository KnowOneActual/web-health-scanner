...
def print_summary(report):
    ...
    console.print(
        Panel(
            f"[bold cyan]SCAN SUMMARY:[/bold cyan] [green]{report['scan_target']}[/green]",
            box=box.DOUBLE,
        )
    )
    ...
    if tech.get("status") == "success" and tech.get("technologies"):
        tech_content = "\n".join([f"• {t}" for t in tech.get("technologies", [])])
    ...
    if links.get("status") == "success":
        ...
        if count > 0:
            broken_links = links.get("broken_links", [])[:3]
            link_content = f"[red]Found {count} broken links.[/red]\n" + "\n".join(
                [f"• {link['status_code']}: {link['url']}" for link in broken_links]
            )
    else:
    ...
    if dns_rep.get("status") == "success":
        ...
        dns_content = "\n".join(
            [
                f"[bold]{k}:[/bold] {', '.join(v[:2])}{'...' if len(v) > 2 else ''}"
    ...
    if nmap.get("status") == "success":
        ...
        res = (
            ", ".join([f"{p['portid']}/{p['service']}" for p in ports])
            if ports
