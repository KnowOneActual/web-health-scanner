...
### Added
- **Command Alias:** Added `whs` as a short-hand command for `web-health-scanner` for easier usage.

### Fixed
- **Documentation:** Updated README to reflect the new `whs` command usage.
...
### Changed
- **Code Quality:** Replaced `black` with `ruff` for all code formatting and linting.
- **Workflow:** Updated GitHub Actions to use `ruff` for faster CI checks.
- **Codebase:** Refactored entire codebase to comply with `ruff` formatting and linting rules.
