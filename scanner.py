    ...
    try:
        ...
        for network in PRIVATE_NETWORKS:
            if ip_obj in network:
                return {
                    "status": "error",
                    "details": f"Resolved IP '{ip_address}' is a private/internal address. SSRF attempt blocked.",
                }
...
def run_subprocess(command, timeout=60):
    ...
    try:
        ...
        print(
            f"[Error] Command not found: '{command[0]}'. Is it installed and in your PATH?",
            file=sys.stderr,
        )
        return None
    except Exception as e:
        print(
            f"[Error] An unexpected error occurred with subprocess: {e}",
            file=sys.stderr,
        )
        return None
...
def parse_nmap_xml(xml_output):
    ...
    try:
        ...
        if host is None:
            return {
                "status": "error",
                "details": "No host information found in nmap output.",
            }
    ...
    try:
        ...
        if "lighthouseResult" not in raw_data:
            if "error" in raw_data:
                error_msg = raw_data.get("error", {}).get(
...
def print_summary(report):
    ...
    if ps.get("status") == "success":
        ...
        print(
            color_status(
                f"  [!] Scan Failed/Skipped: {ps.get('details', 'Unknown error')}"
            )
        )
    ...
    if nmap.get("status") == "success":
        ...
        if not ports:
            ...
            for p in ports:
                print(
                    f"  - Port {p['portid']} ({Fore.GREEN}{p['service']}{Fore.RESET}): {p['state']}"
                )
...
def get_tech_stack(target_url, step_info=""):
    ...
    try:
        ...
        # Case 3: Unexpected Type
            ...
            print(
                f"[Error] webtech scan returned unexpected data type: {type(report)}",
                file=sys.stderr,
            )
            return {
                "status": "error",
                "details": "Webtech scan did not return a valid report object.",
            }

    except ConnectionException:
        print(
            f"[Error] Tech stack scan failed: Connection error for {target_url}",
            file=sys.stderr,
        )
        return {"status": "error", "details": "Connection error."}
    except Exception as e:
        print(
            f"[Error] An unexpected error occurred in get_tech_stack: {e}",
            file=sys.stderr,
        )
        return {"status": "error", "details": f"An unexpected error occurred: {str(e)}"}
...
def get_pagespeed(target_url, api_key, step_info=""):
    ...
    if validation["status"] == "success":  # Only proceed if validation passes
        ...
        if not api_key:
            ...
            print(
                "       [INFO] Get a key: https://developers.google.com/speed/docs/insights/v5/get-started"
            )
        ...
        try:
            ...
            print(
                f"[Error] Could not connect to PageSpeed API: {e}",
                file=sys.stderr,
            )
            ...
            print(
                "[Error] Failed to decode JSON response from PageSpeed API.",
                file=sys.stderr,
            )
...
def analyze_security_headers(target_url, step_info=""):
    ...
    try:
        ...
        print(
            f"[Error] An unexpected error occurred in analyze_security_headers: {e}",
            file=sys.stderr,
        )
        return {"status": "error", "details": f"An unexpected error occurred: {str(e)}"}
...
def get_testssl(target_url, step_info=""):
    ...
    print(
        f"{step_info} [INFO] Running testssl.sh scan on {target_url}... (this may take several minutes)..."
    )
    hostname = urlparse(target_url).hostname
    ...
    try:
        ...
        if os.path.exists(json_output_file) and os.path.getsize(json_output_file) > 0:
            ...
            return {
                "status": "error",
                "details": "testssl.sh did not produce a valid output file (Command failed or timed out after 10 minutes).",
            }

    except json.JSONDecodeError:
        print(
            "[Error] Failed to parse testssl.sh JSON output.",
            file=sys.stderr,
        )
        return {"status": "error", "details": "Failed to parse testssl.sh JSON."}
    except Exception as e:
        print(
            f"[Error] An unexpected error occurred in get_testssl: {e}",
            file=sys.stderr,
        )
        if "json_output_file" in locals() and os.path.exists(json_output_file):
            os.remove(json_output_file)
...
def get_nmap(target_url, step_info=""):
    ...
    try:
        ...
        return {
            "status": "error",
            "details": "nmap command failed or returned no output.",
        }
    except Exception as e:
...
def get_linkchecker(target_url, step_info=""):
    ...
    if validation["status"] == "success":  # Only proceed if validation passes
        try:
            ...
            print(
                f"[Error] Failed to fetch target URL for link check: {e}",
                file=sys.stderr,
            )
            return {
                "status": "error",
                "details": f"Failed to fetch {target_url}: {str(e)}",
            }
        except Exception as e:
            print(
    ...
    try:
        for rtype in record_types:
            try:
                ...
                print(
                    f"[Error] DNS scan failed: Domain not found (NXDOMAIN).",
                    file=sys.stderr,
                )
                return {
                    "status": "error",
                    "details": "Domain not found (NXDOMAIN).",
                }
            except Exception:
        ...
        print(
            f"[Error] An unexpected error occurred in get_dns_records: {e}",
            file=sys.stderr,
        )
        return {"status": "error", "details": f"An unexpected error occurred: {str(e)}"}
...
""",
    ...
    parser.add_argument(
        "url", type=str, help="The target URL to scan (e.g., https://example.com)"
    )
    parser.add_argument(
        "--output",
        type=str,
        default=DEFAULT_OUTPUT_FILENAME,
        help="The filename for the JSON output report.",
    )
    parser.add_argument(
        "--api-key",
        type=str,
    ...
    parser.add_argument(
        "--summary",
        action="store_true",
        help="Print a human-readable summary of results to the terminal.",
    )
    parser.add_argument(
        "--fast",
        action="store_true",
        help="Skip slow scans (nmap and testssl.sh) for a quick check.",
    )
    ...
    # 1. Tech Stack
    master_report["reports"]["tech_stack"] = get_tech_stack(
        args.url, step_info=get_step_info()
    )
    step_counter += 1

    # 2. PageSpeed
    master_report["reports"]["pagespeed"] = parse_pagespeed(
        get_pagespeed(args.url, args.api_key, step_info=get_step_info())
    )
    step_counter += 1

    # 3. Security Headers
    master_report["reports"]["security_headers"] = analyze_security_headers(
        args.url, step_info=get_step_info()
    )
    step_counter += 1
    ...
    if not args.fast:
        master_report["reports"]["testssl"] = get_testssl(
            args.url, step_info=get_step_info()
        )
        step_counter += 1
    else:
        master_report["reports"]["testssl"] = {
            "status": "skipped",
            "details": "Skipped due to --fast flag.",
        }

    # 5. Nmap (Skipped if --fast)
    if not args.fast:
        raw_nmap = get_nmap(args.url, step_info=get_step_info())
        master_report["reports"]["nmap"] = (
            parse_nmap_xml(raw_nmap.get("xml_data"))
            if raw_nmap.get("status") == "success"
            else raw_nmap
        )
        step_counter += 1
    else:
